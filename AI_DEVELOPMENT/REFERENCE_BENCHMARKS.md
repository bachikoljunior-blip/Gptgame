...
- `complete_verified` locally for build-i candidate: the loop boundary now names the sealed loop, newly armed echo count, next loop, and count of recorded paths that will replay. The deterministic handoff criterion passes; four exact 320×568/375×667 WebGL captures cover recorded and replay states with no page, renderer, shell-overflow, control-overlap, undersized-button, narrow-text, or invariant failures. Public delivery remains pending at this checkpoint.
