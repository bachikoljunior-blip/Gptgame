...
export function verifyContinuity({ root = ROOT, read } = {}) {
  ...
  // Deliberately not pinned to one objective id. It used to be, and `main` loosened it in
  // 0aa981d for the reason that assertion always fails eventually: finishing an objective and
  // starting the next one is normal work, and a check that treats it as corruption gets
  // edited out rather than obeyed. What must hold is that an objective id is present and
  // readable — the id itself is the project's business, not the gate's.
  // Anchored to the objective block's own first key. main's loosened form was
  // /objective:[\s\S]*?id: "[^"]+"/, which matches any quoted id anywhere later in the file —
  // deleting the objective id outright still satisfied it, and only the extraction below
...
export function selfTestCases({ root = ROOT } = {}) {
  ...
  return [
    ...
    {
      // Was "the objective id changed", which stopped being a defect when the assertion was
      // loosened. What is still a defect is losing the id, so that is what this drives now.
